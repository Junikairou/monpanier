...
12. ⏳ Section "Personnalisation/Gestion" dans "Plus" : CRUD par l'utilisateur sur types de plat, rayons (catégories courses), catégories de plat (actuellement listes figées dans le code). **Gros chantier identifié, pas encore cadré avec l'utilisateur** (voir note ci-dessous) — pas commencé.

**Note sur le point 12 :** contrairement aux autres demandes de cette liste, celle-ci touche presque tout l'app (les listes de types/rayons/catégories sont des types TypeScript figés + des contraintes `check` en base, utilisés dans une dizaine d'écrans). La rendre éditable par l'utilisateur veut dire remplacer ça par des tables en base (probablement par foyer, à confirmer), perdre la sécurité de typage actuelle, et migrer tous les écrans concernés. C'est plus gros que le chantier "partage du foyer" déjà fait. À ne pas démarrer sans clarifier d'abord la portée avec l'utilisateur.
