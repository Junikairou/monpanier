...
3. ✅ **Bug corrigé et confirmé (2026-08-04) : écran blanc à l'ouverture de création/modification de plat.** Cause : le redimensionnement automatique des zones de texte d'étape de recette (ajouté au lot précédent) provoquait une boucle infinie (`onContentSizeChange` redéclenché à chaque re-render même sans changement réel) → erreur React #185 ("Too many re-renders") → écran blanc. Corrigé en ignorant les mesures qui n'ont pas réellement changé (`src/components/DishForm.tsx`, `StepRow`).
